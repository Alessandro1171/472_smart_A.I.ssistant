...
def pixel_intensity_pre_class(paths_train: list, paths_test: list, classes: list):
    """
    ...
    for class_counter in range(0, len(classes)):
        fig.add_subplot(sides, sides, (class_counter + 1))
        plt.axis('off')
        # construct histogram for specific class
        for sample_pool in range(0, min(sample_size)):
            image = cv2.imread(random.choice(full_paths[class_counter]))
        ...
        title: str = "Pixel class Intensity Distribution:" + classes[class_counter]
        plt.title(title)
    plt.show()


def pixel_intensity_pre_class_v2(paths_train: list, paths_test: list, classes: list, colors: list):
    """
    plots the aggregated pixel intensity distribution of images for each class in the form of a histogram
