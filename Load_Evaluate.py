...
class Get_Image(Dataset):
    """
    Gets a single image from specified path and labels it as the given class
    """
...
def test_set_evaluation(model):
    """
    evaluates a models performance on a test set and then displays a confusion matrix based on the predicted and actual
...
def display_values(all_preds, all_labels, test_acc):
    """
    calculates and displays the evaluation metrics for the model both the micro and macro metrics
    additionally displays a classification report detailing the metrics for each class
    :param all_preds all predictions made by the model
    :param all_labels all actual labels made by the class
    :param test_acc the accuracy of the model
    """
